...
type UserRegisteredPayload struct {
	...
	// Username is the handle auth-service assigned at registration. The
	// projection stores THIS handle rather than deriving a second one:
	// app.users is what /v1/users/by-username answers from, so the two
	// drifting apart breaks every profile link.
