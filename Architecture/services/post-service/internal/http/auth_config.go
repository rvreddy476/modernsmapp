...
// AudiencePost is the audience post-service accepts on service tokens.
const AudiencePost = "post"
...
// ServiceCallersFromEnv builds the service-token verifier for post-service's
// token-only routes (today: the admin family, admin_token.go). Same shape as
// food-service, dating-service and payments-service:
//
...
//	SERVICE_CALLER_ADMIN_SERVICE_OPS=social:stats.read,social:posts.moderate,...
//
...
func ServiceCallersFromEnv(getenv func(string) string) (*servicetoken.Verifier, error) {
	...
	v := servicetoken.NewVerifier(AudiencePost)
