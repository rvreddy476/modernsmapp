...
// two-person rules and writes its own audit row; then it calls group-service
...
//
// Scope is deliberately small: the group report queue (list, detail) and a
// decision (uphold or dismiss, with a reason), plus stats. A decision does not
// enforce anything — removing a member, deleting a post or archiving a group
// are owner flows that sync chat membership and are not wired here.
package http
...
// Chat admin permissions for groups. Names identity's catalogue already has
...
// to group-service. Deployment registers admin-service with exactly these.
...
const ctxAdminActor = "group_admin_actor"
...
// no evidence here.
const InternalAdminPrefix = "/v1/groups/internal/admin"

// GroupAdmin is what the admin handlers need; the store in production.
type GroupAdmin interface {
	AdminListGroupReports(ctx context.Context, status string, limit int, cursor string) ([]store.GroupReport, string, error)
	AdminGetGroupReport(ctx context.Context, id uuid.UUID) (*store.GroupReport, error)
	AdminDecideGroupReport(ctx context.Context, actor, id uuid.UUID, status, reason string) (*store.GroupReport, error)
	AdminGroupStats(ctx context.Context) (*store.GroupAdminStats, error)
}
...
func (h *Handler) WithAdmin(a GroupAdmin) *Handler {
	h.admin = a
	return h
}
...
func (h *Handler) requireAdminToken(perm string) gin.HandlerFunc {
	if perm == "" {
		panic("group-service: requireAdminToken needs the route's permission")
	}
	return func(c *gin.Context) {
		...
		if errors.Is(err, servicetoken.ErrScopeDenied) {
			slog.WarnContext(ctx, "group-service: admin token lacks the route permission", "required", perm, "path", c.Request.URL.Path)
		...
		if err != nil {
			slog.WarnContext(ctx, "group-service: admin token refused", "reason", err.Error(), "path", c.Request.URL.Path)
		...
		if verified.Issuer != IssuerAdminService {
			slog.WarnContext(ctx, "group-service: admin route called by a non-admin service", "issuer", verified.Issuer, "path", c.Request.URL.Path)
			api.ErrorWithContext(ctx, c.Writer, http.StatusForbidden, CodeServiceTokenRejected, "service token rejected", nil)
			c.Abort()
			return
		}
...
func writeAdminError(c *gin.Context, err error) {
	...
	default:
		slog.ErrorContext(ctx, "group-service: admin action failed", "error", err, "path", c.Request.URL.Path)
...
func (h *Handler) AdminStats(c *gin.Context) {
	...
	out, err := h.admin.AdminGroupStats(c.Request.Context())
	if err != nil {
...
func (h *Handler) AdminListReports(c *gin.Context) {
	...
	items, next, err := h.admin.AdminListGroupReports(c.Request.Context(), c.Query("status"), limit, c.Query("cursor"))
	if err != nil {
		writeAdminError(c, err)
		return
	}
...
func (h *Handler) AdminGetReport(c *gin.Context) {
	...
	out, err := h.admin.AdminGetGroupReport(c.Request.Context(), id)
	if err != nil {
		writeAdminError(c, err)
		return
	}
...
func (h *Handler) AdminDecideReport(c *gin.Context) {
	...
	out, err := h.admin.AdminDecideGroupReport(c.Request.Context(), actor, id, status, reason)
	if err != nil {
		writeAdminError(c, err)
		return
	}
