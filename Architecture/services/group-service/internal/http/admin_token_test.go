// Admin-service token tests (admin console Wave 2 — Chat, groups). No
...
const adminTestInternalKey = "group-admin-token-test-key"
...
func (f *adminFakeBackend) AdminListGroupReports(context.Context, string, int, string) ([]store.GroupReport, string, error) {
	f.calls++
	return []store.GroupReport{}, "", nil
}

func (f *adminFakeBackend) AdminGetGroupReport(_ context.Context, id uuid.UUID) (*store.GroupReport, error) {
	f.calls++
	return &store.GroupReport{ID: id}, nil
}

func (f *adminFakeBackend) AdminDecideGroupReport(_ context.Context, actor, id uuid.UUID, status, _ string) (*store.GroupReport, error) {
	f.calls++
	f.actors = append(f.actors, actor)
	return &store.GroupReport{ID: id, Status: status}, nil
}

func (f *adminFakeBackend) AdminGroupStats(context.Context) (*store.GroupAdminStats, error) {
	f.calls++
	return &store.GroupAdminStats{}, nil
}
...
func adminTokenRouter(be GroupAdmin, v *servicetoken.Verifier, key string) *gin.Engine {
...
const adminBody = `{"decision":"uphold","reason":"harassment in group chat"}`
...
func TestGroupAdminToken_RightScopeAdmittedActorIsAct(t *testing.T) {
...
func TestGroupAdminToken_Refusals(t *testing.T) {
	rg := newAdminTokenRig(t)
...
func TestGroupAdminToken_KeyAndForgedActorRefused(t *testing.T) {
	rg := newAdminTokenRig(t)
	var checked int
...
func TestGroupAdminToken_EveryRouteNeedsItsPermission(t *testing.T) {
	rg := newAdminTokenRig(t)
...
func TestGroupAdminToken_KeyIndependentAndKeyRoutesUnchanged(t *testing.T) {
	rg := newAdminTokenRig(t)
	...
	if w := adminServe(rg.r, http.MethodGet, "/v1/groups/discover", "", bearer(tok)); w.Code != http.StatusUnauthorized {
		t.Fatalf("key route with a token but no key: status=%d, want 401", w.Code)
	}
}

func TestGroupAdminToken_NoVerifierRefuses(t *testing.T) {
	rg := newAdminTokenRig(t)
...
func TestGroupServiceCallersFromEnv(t *testing.T) {
