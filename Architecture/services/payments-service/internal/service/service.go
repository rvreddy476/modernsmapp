...
//
// Deprecated: InitiateRefund is replaced by RequestRefund.
//
...
// call, and only a verified provider outcome advances it to settled. See
// p0.go.
//
// This shim exists solely so a caller that has not migrated fails loudly at
// compile time rather than silently keeping the old semantics.
func (s *Service) InitiateRefund(context.Context, uuid.UUID, uuid.UUID, int64, string) (*postgres.PaymentIntent, error) {
	return nil, fmt.Errorf(
		"payments: InitiateRefund is removed (Commerce P0 A6) — use RequestRefund, which persists a durable " +
			"refund command with a provider idempotency key before contacting the provider")
}
