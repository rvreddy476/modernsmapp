...
// Module 3 SR-1 — the complete edge verifier, extracted from cmd/server.
//
// It lives here for two reasons:
//
//  1. The root .gitignore contains a bare `server` rule, so any NEW file under
//     cmd/server/ is untracked. Code that only exists in a working tree is
//     code that fails on a clean checkout.
//  2. auth-service mints the tokens this function verifies. Until the verifier
//     was importable, the only way to "prove" the two agreed was to reimplement
//     one side inside the other's test — which proves the reimplementation, not
//     the contract. auth-service's contract test now calls THIS function with a
//     token produced by its real mint path.
//
// Why RS256: with a shared HS256 secret, every verifier can MINT platform
// tokens. With RS256, only auth-service holds the private key; the gateway
// holds the public key and can verify but never mint. HS256 stays supported
// outside production so tokens minted before the cutover keep working.

// KeySet holds the verification material.
//
// ActiveKID signs (auth-service only); both active and previous are accepted
// on verify so a kid rotation has a window where prior tokens stay valid. A
// token with no `kid` header (legacy) falls back to the active key.
type KeySet struct {
	ActiveKID      string
	ActiveSecret   string
	PreviousKID    string
...
// Identity is what a verified token yields. Every field becomes a trusted
// downstream header, so nothing here may originate from a client.
...
// callers over HTTP — the edge answers a flat 401 so a probing client learns
...
func Verify(tokenStr string, keys KeySet, policy Policy, now time.Time) (Identity, error) {
	...
	// Only HS256 and RS256 are accepted: no `none`, no alg confusion. The
	// algorithm is taken from the header but is then re-checked against the
	// policy below, so a downgrade to HS256 cannot slip past in production.
	switch header.Alg {
	case "HS256":
