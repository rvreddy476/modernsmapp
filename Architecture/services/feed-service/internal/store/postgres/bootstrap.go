...
// BootstrapSchema applies the base feed-service schema, then runs any migration
...
func BootstrapSchema(ctx context.Context, db *pgxpool.Pool, schemaSQL string, migrations fs.FS) error {
	...
	if migrations != nil {
		if err := migrationrunner.Run(ctx, db, "feed-service", migrations, "migrations"); err != nil {
			return fmt.Errorf("apply feed migrations: %w", err)
		}
	}
	return nil
}
