...
func main() {
	...
	// (/v1/qa/internal/admin/*): SERVICE_CALLERS=admin-service plus
	...
	// or empty ops refuses to start.
	serviceVerifier, err := http.ServiceCallersFromEnv(os.Getenv)
	if err != nil {
		slog.Error("qa-service: SERVICE_CALLERS", "error", err)
		os.Exit(1)
	}
	if serviceVerifier == nil {
		slog.Warn("qa-service: SERVICE_CALLERS not set — admin-service tokens are refused; /v1/qa/internal/admin answers 401")
	} else {
		slog.Info("qa-service: service-token callers registered", "callers", serviceVerifier.Callers())
	}
	qaHandler.WithServiceAuth(serviceVerifier)
