...
func TestLifecycleErrorMapping(t *testing.T) {
	...
	cases := []struct {
		...
		{"out of range", http.MethodPost, "/v1/food/orders", `{"address_id":"` + uuid.NewString() + `","payment_method":"upi"}`,
			postgres.ErrAddressOutOfRange, http.StatusUnprocessableEntity, "FOOD_ADDRESS_OUT_OF_RANGE"},
		{"outside hours", http.MethodPost, "/v1/food/orders", `{"address_id":"` + uuid.NewString() + `","payment_method":"upi"}`,
			postgres.ErrRestaurantOutsideHours, http.StatusUnprocessableEntity, "FOOD_RESTAURANT_OUTSIDE_HOURS"},
		{"address location", http.MethodPost, "/v1/food/orders", `{"address_id":"` + uuid.NewString() + `","payment_method":"upi"}`,
			postgres.ErrAddressLocationRequired, http.StatusUnprocessableEntity, "FOOD_ADDRESS_LOCATION_REQUIRED"},
		{"restaurant location", http.MethodPost, "/v1/food/orders", `{"address_id":"` + uuid.NewString() + `","payment_method":"upi"}`,
			postgres.ErrRestaurantLocationMissing, http.StatusUnprocessableEntity, "FOOD_RESTAURANT_LOCATION_MISSING"},
		{"not accepting", http.MethodPost, "/v1/food/orders", `{"address_id":"` + uuid.NewString() + `","payment_method":"upi"}`,
