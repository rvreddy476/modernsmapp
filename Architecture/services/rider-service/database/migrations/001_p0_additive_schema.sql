-- rider-service migration 001: P0 additive schema (quotes, dispatch attempts, inbox, outbox, payment reconciliation, safety actions, revision concurrency).
...
-- Before the one-active-ride rules below can be enforced, resolve rows that
-- already violate them (older dev data, aborted test runs): keep the newest
-- active ride per customer / per partner and expire the rest, with a history
-- row so the change is traceable. Live pilots never ran with this data.
WITH ranked AS (
    SELECT id, status,
           ROW_NUMBER() OVER (PARTITION BY customer_user_id ORDER BY created_at DESC, id DESC) AS rn
    FROM rider_rides
    WHERE status NOT IN ('completed','cancelled_by_customer','cancelled_by_partner','cancelled_by_admin','expired','failed')
), expired AS (
    UPDATE rider_rides r SET status = 'expired', updated_at = NOW()
    FROM ranked WHERE r.id = ranked.id AND ranked.rn > 1
    RETURNING r.id, ranked.status::text AS from_status
)
INSERT INTO rider_ride_status_history (ride_id, from_status, to_status, actor_kind, reason)
SELECT id, from_status, 'expired', 'system', 'migration 001: duplicate active ride for customer' FROM expired;
...
INSERT INTO rider_ride_status_history (ride_id, from_status, to_status, actor_kind, reason)
SELECT id, from_status, 'expired', 'system', 'migration 001: duplicate active ride for partner' FROM expired;
