...
// AudienceRider is the audience rider-service (Mopedu) accepts on service
// tokens.
const AudienceRider = "rider"
...
// ServiceCallersFromEnv builds the service-token verifier for rider's
...
//	SERVICE_CALLER_ADMIN_SERVICE_OPS=rider:stats.read,rider:partners.read,...
//
...
func ServiceCallersFromEnv(getenv func(string) string) (*servicetoken.Verifier, error) {
	...
	v := servicetoken.NewVerifier(AudienceRider)
