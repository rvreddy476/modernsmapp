...
// FindServiceableCity finds the closest active city whose geofence radius covers (lat, lng).
func (s *Store) FindServiceableCity(ctx context.Context, lat, lng float64) (*City, error) {
	cities, err := s.ListActiveCities(ctx)
	if err != nil {
		return nil, err
	}
	for _, city := range cities {
		if city.CenterLat != nil && city.CenterLng != nil {
			radius := 40.0
			if city.RadiusKM != nil && *city.RadiusKM > 0 {
				radius = *city.RadiusKM
			}
			dist := geo.HaversineKM(lat, lng, *city.CenterLat, *city.CenterLng)
			if dist <= radius {
				return &city, nil
			}
		}
