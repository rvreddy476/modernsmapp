...
// The StalePayoutDetector that used to live here — an hourly log line for
// rows in a status ('in_flight') that no longer exists — was replaced in
// plan Phase 4C by the reconciler in payout_rail.go, which fetches the
// provider's view and converges the row instead of logging it.
