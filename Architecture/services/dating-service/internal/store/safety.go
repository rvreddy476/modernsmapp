	...
	rows, err := s.db.Query(ctx, `
        ...
        WHERE id IN (
            ...
            WHERE reminder_fired_at IS NULL
              ...
              -- Lane D8: reminders go only to the two participants, and
              -- never once either has blocked the other.
...
// ErrPanicNotFound is returned when a panic incident id does not exist.
var ErrPanicNotFound = errors.New("not_found: panic incident not found")
