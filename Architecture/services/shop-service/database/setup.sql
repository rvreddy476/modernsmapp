...
CREATE INDEX IF NOT EXISTS idx_storefronts_seller ON shop.storefronts(seller_id);
CREATE INDEX IF NOT EXISTS idx_storefronts_handle ON shop.storefronts(handle);

CREATE TABLE IF NOT EXISTS shop.storefront_featured (
    storefront_id   UUID NOT NULL REFERENCES shop.storefronts(id) ON DELETE CASCADE,
    listing_id      UUID NOT NULL,
    position        INT NOT NULL,
    PRIMARY KEY (storefront_id, position)
);

CREATE TABLE IF NOT EXISTS shop.storefront_collections (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    storefront_id   UUID NOT NULL REFERENCES shop.storefronts(id) ON DELETE CASCADE,
    name            TEXT NOT NULL,
    cover_media_id  UUID,
...
CREATE TABLE IF NOT EXISTS shop.collection_listings (
    collection_id   UUID NOT NULL REFERENCES shop.storefront_collections(id) ON DELETE CASCADE,
    listing_id      UUID NOT NULL,
    position        INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS shop.post_product_tags (
    id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_product_tags_post ON shop.post_product_tags(post_id);
CREATE INDEX IF NOT EXISTS idx_product_tags_listing ON shop.post_product_tags(listing_id);
...
CREATE INDEX IF NOT EXISTS idx_wishlists_user ON shop.wishlists(user_id);
...
CREATE INDEX IF NOT EXISTS idx_wishlist_items_listing ON shop.wishlist_items(listing_id);
...
CREATE INDEX IF NOT EXISTS idx_group_buys_listing ON shop.group_buys(listing_id, status);

CREATE TABLE IF NOT EXISTS shop.group_buy_participants (
...
-- ─── Ads ─────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS shop.ad_campaigns (
    id                      UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_campaigns_advertiser ON shop.ad_campaigns(advertiser_id, status);
CREATE INDEX IF NOT EXISTS idx_campaigns_active ON shop.ad_campaigns(status, starts_at) WHERE status = 'active';

CREATE TABLE IF NOT EXISTS shop.ad_sets (
    id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    campaign_id  UUID NOT NULL REFERENCES shop.ad_campaigns(id) ON DELETE CASCADE,
    name        TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS shop.ad_creatives (
    id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    campaign_id  UUID NOT NULL REFERENCES shop.ad_campaigns(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS shop.ad_performance (
    campaign_id UUID NOT NULL REFERENCES shop.ad_campaigns(id),
    creative_id UUID REFERENCES shop.ad_creatives(id),
    date        DATE NOT NULL,
...
CREATE TABLE IF NOT EXISTS shop.ad_frequency_caps (
    campaign_id           UUID NOT NULL REFERENCES shop.ad_campaigns(id),
