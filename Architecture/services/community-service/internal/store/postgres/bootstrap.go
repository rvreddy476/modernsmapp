...
// BootstrapSchema applies the base community-service schema, then runs any
...
func BootstrapSchema(ctx context.Context, db *pgxpool.Pool, schemaSQL string, migrations fs.FS) error {
	...
	if _, err := db.Exec(ctx, schemaSQL); err != nil {
		return fmt.Errorf("apply community schema: %w", err)
	}
	if migrations != nil {
		if err := migrationrunner.Run(ctx, db, "community-service", migrations, "migrations"); err != nil {
			return fmt.Errorf("apply community migrations: %w", err)
		}
	}
	return nil
}
