 ...
 */
fun MediaUrlResolver.playbackFor(item: FeedItem): Playback? =
    item.media.firstOrNull { it.kind == VIDEO_KIND }?.let { playbackFor(it) }

fun MediaUrlResolver.playbackFor(media: FeedMedia): Playback? {
 ...
 */
fun FeedItem.hasVideo(): Boolean = media.any { it.kind == VIDEO_KIND }
