...
class FeedApiRequestTest {
    ...
    /** Tube's list: the `videos` surface, paged like the others, with no narrowing. */
    @Test
    fun `videos asks for the plain videos surface`() {
        val target = feedRequest(FeedQuery.Videos)

        assertThat(target).startsWith("/v1/feed/videos?")
        assertThat(target).contains("limit=15")
        assertThat(target).doesNotContain("following_only")
        assertThat(target).doesNotContain("circle_only")
    }
