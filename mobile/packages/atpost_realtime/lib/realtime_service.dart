...
final realtimeServiceProvider = Provider<RealtimeService>((ref) {
  // Bound to the app's AuthService via authSessionProvider (see the
  // root ProviderScope overrides).
  final auth = ref.watch(authSessionProvider);
  final service = RealtimeService(auth);

  void syncAuth() {
    if (auth.isAuthenticated && (auth.token?.isNotEmpty ?? false)) {
      service.connect();
    } else {
      service.disconnect();
    }
  ...
  syncAuth();
  final authSub = auth.sessionChanges.listen((_) => syncAuth());
