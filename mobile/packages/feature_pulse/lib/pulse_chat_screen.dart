...
import 'package:feature_pulse/safety/chat_safety_sheet.dart';
import 'package:feature_pulse/safety/panic_sheet.dart';
import 'package:feature_pulse/safety/report_block_sheet.dart';
import 'package:feature_pulse/safety/share_location_banner.dart';
import 'package:feature_pulse/widgets/match_context_banner.dart';
import 'package:feature_pulse/widgets/moderation_banner.dart';
import 'package:feature_pulse/providers/pulse_providers.dart';
import 'package:feature_pulse/services/pulse_auth_service.dart';
import 'package:feature_pulse/services/pulse_chat_outbox.dart';
import 'package:feature_pulse/services/pulse_crash_breadcrumbs.dart';
import 'package:feature_pulse/services/pulse_telemetry.dart';
...
class _PulseChatScreenState extends ConsumerState<PulseChatScreen> {
  ...
  StreamSubscription<PulseChatWireMessage>? _wsSub;
  ...
  void _ensureSubscriptions() {
    _wsSub ??= ref.read(pulseChatEventsProvider).listen(_onWsEvent);
    if (_outboxSub == null) {
  ...
  void _onWsEvent(PulseChatWireMessage event) {
    if (event.conversationId != widget.conversationId) return;
    if (!mounted) return;
    // The host maps its realtime channel's `message.new` wire type into
    // PulseChatWireMessage (see host/pulse_host.dart + the app override).
