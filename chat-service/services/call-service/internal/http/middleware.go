...
func getUserID(c *gin.Context, log *slog.Logger) (uuid.UUID, bool) {
	...
	if !ok || idStr == "" {
		api.Error(c.Writer, http.StatusUnauthorized, "UNAUTHORIZED", "Invalid user context", nil, nil)
		return uuid.Nil, false
	}
	id, err := uuid.Parse(idStr)
	if err != nil {
		api.Error(c.Writer, http.StatusUnauthorized, "UNAUTHORIZED", "Invalid user ID format", nil, nil)
		return uuid.Nil, false
	}
	return id, true
}
