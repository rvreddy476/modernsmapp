...
// ProfileHandler is the interface the consumer uses to act on events.
type ProfileHandler interface {
	CreateProfile(ctx context.Context, userID uuid.UUID, firstName, lastName, dob, gender string) error
}

// Consumer reads events from Kafka and applies them to the profile-service.
...
func NewConsumerWithDialer(brokers []string, topic, groupID string, dialer *kafka.Dialer, db *pgxpool.Pool, handler ProfileHandler, logger *slog.Logger) *Consumer {
	if logger == nil {
		logger = slog.Default()
	}
...
func (c *Consumer) Start(ctx context.Context) {
	c.log.Info("starting profile-service kafka consumer")
	for {
		msg, err := c.reader.ReadMessage(ctx)
		if err != nil {
			if ctx.Err() != nil {
				c.log.Info("profile-service kafka consumer stopped")
				return
			}
...
func (c *Consumer) handle(ctx context.Context, msg kafka.Message) error {
	...
	err := c.db.QueryRow(ctx,
		"SELECT EXISTS(SELECT 1 FROM profile.inbox_events WHERE consumer_name=$1 AND event_id=$2)",
		profileConsumerName, eventID,
	).Scan(&exists)
	if err == nil && exists {
	...
	c.db.Exec(ctx,
		"INSERT INTO profile.inbox_events(consumer_name, event_id) VALUES($1,$2) ON CONFLICT DO NOTHING",
		profileConsumerName, eventID,
	)
	return nil
}
...
func (c *Consumer) dispatch(ctx context.Context, env sharedEvents.EventEnvelope) error {
	...
	case sharedEvents.UserRegistered:
		...
		if err := c.handler.CreateProfile(ctx, userID, p.FirstName, p.LastName, p.DOB, p.Gender); err != nil {
			return err
		}
