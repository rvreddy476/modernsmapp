...
func TestPurgeIsIdempotentAndAcksEveryTime(t *testing.T) {
	...
	h := NewHandler("user", st, acks, st, nil).WithClock(func() time.Time { return fixed })
	...
	if acks.acks[0].Service != "user" || acks.acks[0].UserID != id.String() || !acks.acks[0].PurgedAt.Equal(fixed) {
...
func TestNoAckWhenEraseFails(t *testing.T) {
	...
	acks := &fakeAcks{log: &order}
	h := NewHandler("user", st, acks, st, nil)
	if err := h.Handle(context.Background(), EventUserPurgeRequested, payload(id)); err == nil {
...
func TestHideAndUnhideToggle(t *testing.T) {
	...
	h := NewHandler("user", st, &fakeAcks{log: &order}, st, nil)
