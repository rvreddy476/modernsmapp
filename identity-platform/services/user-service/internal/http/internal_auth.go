...
// every gated request. Audit UC1: previously user-service had no internal-
// key gate at all — every endpoint was reachable directly without going
// through the API gateway, which meant X-User-Id was effectively a
// public header that any caller could spoof.
//
// Empty secret keeps dev unblocked (the wiring in main.go logs a loud
// WARN when the env var is unset). Mirrors the auth-service middleware
// of the same name; inlined here because identity-shared doesn't yet
// depend on gin.
