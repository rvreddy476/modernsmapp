...
func Run(ctx context.Context, db *pgxpool.Pool, service string, fsys fs.FS, subdir string) error {
	...
	for _, name := range names {
		...
		// path.Join, not string concatenation: io/fs rejects paths that are
		// not clean and unrooted, so a subdir of "." would produce
		// "./016_x.sql" and fail with "invalid argument". That matters here
		// because this runner is fed os.DirFS(dir) with subdir ".", unlike
		// the embed.FS callers that always pass "migrations".
		filePath := pathpkg.Join(subdir, name)
		...
		if err != nil {
			return fmt.Errorf("migrationrunner: read %s: %w", filePath, err)
		}
