...
All history adds 20 more `no_statement_row` rows, on 2025-03-12, 2025-04-09, 2026-01-15 and 2026-01-16, totalling 157,000 dashboard views. These are the **M-01 fixture summary rows**: `content_daily_summary` rows with no `content_hourly_agg` beneath them (20 rows across those four days). The January remediation reversed the 19 earnings that were priced from them and deleted the rate and band fixtures; it did not delete the summary rows themselves, and the rollup will not, because those days are frozen. They are harmless to money now (nothing accrues on a reversed slot, and the creators are test accounts) but they will show here on every run until removed. Removing them is a `?force=1` rerun of those four days on the internal aggregate route — delete-then-insert over zero hourly rows ends with zero summary rows — and is a write to the live development analytics tables, so it waits for the same authorisation the remediation had. Staging and production: run `03_contamination_check_other_envs.sql` first; if the rows exist there, they must go before anything accrues.
