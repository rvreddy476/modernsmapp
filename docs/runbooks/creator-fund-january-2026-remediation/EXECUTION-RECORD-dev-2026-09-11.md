...
## Forced rerun of four frozen days 2026-09-11T20:25:06 UTC — M-01 fixture summary rows removed (dev only, founder-authorised "go ahead with the forced rerun on dev")

Before: 20 `content_daily_summary` rows on 2025-03-12 (4), 2025-04-09 (4), 2026-01-15 (9), 2026-01-16 (3), 157,000 views, zero hourly rows, zero raw events, zero sessions beneath them; snapshot `m01_summary_rows_before_forced_rerun.csv` sha256 `255bbf39…`. Route `POST /v1/analytics/internal/aggregate?day=…&force=1` on analytics-service (127.0.0.1:8094, build `c39c84e7`): 401 without key, 409 DAY_FROZEN without force, then 200 ×4 (`hours_rebuilt:24, rolled_up:true, forced:true`), container logged FORCED REWRITE for each day. After: 0 summary rows on the four days, hourly total unchanged (132), summary total 78→58, `rollup_progress` rows for the four days = frozen, M-01 proof (summary rows with no hourly events) = 0, reconciliation: 8 match / 3 explained unaccrued (2026-09-07) / 3 open-day, no `mismatch`. First attempt sent the four calls to 127.0.0.1:8086 (not analytics) and got 404s: nothing ran. Log `exec/forced_rerun_m01_summary_rows.log`.
